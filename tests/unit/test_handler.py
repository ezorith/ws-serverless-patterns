# Add your unit testing code here# Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
...
def test_update_user_by_id():
    with my_test_environment():
        ...
        event = {
            ...
            "httpMethod": "PUT",
            "pathParameters": {"userid": UUID_MOCK_VALUE_JOHN},
            "body": '{"name":"John Updated"}'
        }
        ret = users.lambda_handler(event, '')
        assert ret['statusCode'] == 200
        data = json.loads(ret['body'])
        assert data['userid'] == UUID_MOCK_VALUE_JOHN
...
def test_unsupported_route():
    with my_test_environment():
        ...
        assert json.loads(ret['body'])['Message'] == 'Unsupported route'
